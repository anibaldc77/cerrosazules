...
if ($Accion==1){
	...
	if ($check == $_POST['check']){
		...
		if($clave==$clave1){
			...
			include_once("../../clases/sysbarrio/pedidousuario.class.php");		

			$ipusuario = $funcion->dirIP($_SERVER);	

			$pedidousuario= new Pedidousuario();
			$ipusuario = $funcion->dirIP($_SERVER);
			$idregistro=$pedidousuario->createnew_pedidousuario("", $nombre, $email, $telefono, $clave, $ipusuario,0, date('Y-m-d h:i:s a'), "", $check, date('Y-m-d h:i:s a'));
			if($idregistro>0){
					$mensajeMuestra['titulo']="Solicitud Ingresada";
					$mensajeMuestra['text']="Los Datos serán verificados, una vez realizado este proceso se le informará al correo electrónico su habilitación";
    ...
    <div class="col s12 z-depth-4 card-panel">
      ...
      <form id="formLogin" name="formLogin" class="login-form"  action="" method="post" autocomplete="off" novalidate>
       ...
       <div class="row">
          ...
          <div class="row">
            <div class="input-field col s12">
	            <button class="btn orange waves-effect waves-light col s12" type="submit" value="1" name="Accion"  id="Accion" onClick="convierteMd5(this.form)">Registrarse
...
/*	function convierteMd5(formulario){
	...
	function convierteMd5(formulario){
		...
		email = document.getElementById("email").value;
		clave = document.getElementById("clave").value;
		resultado = md5convert(nombre + email + clave);
		//resultado = md5(nombre + email + clave+text);
		//alert("El Hash MD5 es:\n" + resultado);
		document.getElementById("check").value = resultado
	}		
